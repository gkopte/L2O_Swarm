...
def special_fitness_function():
    def f(X):
        with tf.Session() as sess:
            sess.run(tf.get_default_graph().finalize())
            sess.run(f, feed_dict={"x_0:0": X})
            # sess.run(tf.assign(x,X))
            return sess.run(problem)
    return f

x_val = tf.random_uniform([10, 2], minval=-3, maxval=3,dtype=tf.float32)
v_val = tf.zeros([10, 2],dtype=tf.float32)
opt = pso(fitness_fn=special_fitness_function(),pop_size=10, dim=2, n_iter=128,x_min=-3,x_max=3,x_init=x_val, v_init=v_val)
opt.train()
